...
- 📊 **实时进度与状态（v0.9.7）**：启动横幅（版本 / 模式 / 深度 / 线程）、耗时探测的
  **加载指示器**、扫描进度条（**已找到 N 项 / X MB** + **剩余时间估算**，并行扫描也按
  完成顺序实时刷新）、清理时显示**当前目标 + 已处理/跳过计数 + 已释放/已入回收站字节**。
  全部走 stderr 且 **TTY 限定**——`--json`、管道、MCP 场景一个字节都不多出。
