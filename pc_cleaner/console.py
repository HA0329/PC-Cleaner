...
def enable_ansi() -> bool:
    ...
    v0.9.10：**非 TTY 一律不出色**。此前 Windows 分支已经这么判，但 POSIX 分支
    写成"无条件返回 True"，于是管道/重定向场景仍会输出 ``\\x1b[1m``；配合
    ``display_width`` 当时不剥离转义码的实现，表格宽度会被算成 83/92/93 这种
    "每行都不一样"的结果（CI 日志里就是这样暴露的）。
    现在两条分支语义一致：只有 stdout/stderr 都是 TTY 才启用颜色，
    非 TTY（管道 / 重定向 / CI）一律纯文本。
    """
    try:
...
def display_width(text: str) -> int:
    ...
    v0.9.10 修复：先剥离 ANSI 转义序列。此前直接逐字符累加，于是
    ``\\x1b[1m  1.\\x1b[0m`` 这种带色单元格会把 ``\\x1b[1m`` 当成 4 个可见列，
    边框（无色）/表头/数据行算出的宽度各不相同 —— 80 列窗口里表格会被折行撕碎。
